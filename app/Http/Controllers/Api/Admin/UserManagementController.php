...
class UserManagementController extends Controller
{
    ...
    public function dashboard()
    {
        $stats = [
            ...
            'papers_under_review' => \App\Models\Paper::whereIn('status', ['submitted', 'under_review'])->count(),
            'papers_accepted' => \App\Models\Paper::where('status', 'accepted')->count(),
            'published' => \App\Models\Paper::where('is_published', true)->count(),
            'rejected' => \App\Models\Paper::where('status', 'rejected')->count(),
            'revision_required' => \App\Models\Paper::where('status', 'revision_required')->count(),
            'draft_papers' => \App\Models\Paper::where('status', 'draft')->count(),
            'pending_papers' => \App\Models\Paper::whereIn('status', ['submitted', 'under_review', 'revision_required'])->count(),
            'total_reviews' => \App\Models\Review::where('status', 'completed')->count(),
            'submitted_today' => \App\Models\Paper::whereDate('submission_date', today())->count(),
            'reviews_today' => \App\Models\Review::where('status', 'completed')->whereDate('submitted_at', today())->count(),
            'decisions_today' => \App\Models\Paper::whereDate('decision_date', today())->count(),
            'active_reviewers' => User::role('reviewer')->where('status', 'active')->whereHas('reviewerProfile', function ($q) {
